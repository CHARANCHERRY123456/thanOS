...
int findMaximumPriorityProcess(Process processes[], int n) {
    int max = -INF, pos = -1;
    for (int i = 0; i < n; i++) {
        if (processes[i].isInReadyQ && processes[i].priority > max) {
...
void priorityScheduling(Process processes[], int n) {
    ...
    int i = 0;
    while ((i = findMaximumPriorityProcess(processes, n)) != -1) {
...
void main() {
    ...
    for (int i = 0; i < n; i++) {
        processes[i].pid = i + 1;
        printf("Enter arrival time and burst time and priority for process %d: ", i + 1);
